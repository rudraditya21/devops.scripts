...
## Purpose
Generate service configuration for `zabbix` monitoring workflows.

## Location
`monitoring/zabbix/configure.sh`
...
## Usage
```bash
monitoring/zabbix/configure.sh --config-file /tmp/zabbix.yml --endpoint http://monitoring.internal --dry-run
```
