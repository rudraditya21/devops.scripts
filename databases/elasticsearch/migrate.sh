...
Apply elasticsearch migrations and record state.

Options:
  --migrations-dir PATH    Directory with migration files (required)
  --target VERSION         Target version label (default: latest)
  --state-file PATH        State file path (default: .state/elasticsearch-migrations.state)
...
migrations_dir=""
target="latest"
state_file=".state/elasticsearch-migrations.state"
dry_run=false
