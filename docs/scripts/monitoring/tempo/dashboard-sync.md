...
## Purpose
Sync dashboard files for `tempo` monitoring workflows.

## Location
`monitoring/tempo/dashboard-sync.sh`
...
## Usage
```bash
monitoring/tempo/dashboard-sync.sh --source ./dashboards --destination /tmp/tempo-dashboards --folder platform --dry-run
```
