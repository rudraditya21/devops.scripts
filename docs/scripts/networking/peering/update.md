...
## Purpose
Run `update` for `peering` networking objects using validated flags.

## Location
`networking/peering/update.sh`
...
## Usage
```bash
networking/peering/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
