...
Run delete for loadbalancer networking objects.
...
if $json; then
  printf '{"domain":"loadbalancer","action":"delete","name":"%s","target":"%s","cidr":"%s","dry_run":%s}\n' \
    "$(json_escape "$name")" \
    "$(json_escape "$target")" \
    "$(json_escape "$cidr")" \
    "$dry_run"
...
if $dry_run; then
  printf 'DRY-RUN: would delete loadbalancer object %s\n' "$name" >&2
fi
