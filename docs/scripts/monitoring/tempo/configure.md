...
## Purpose
Generate service configuration for `tempo` monitoring workflows.

## Location
`monitoring/tempo/configure.sh`
...
## Usage
```bash
monitoring/tempo/configure.sh --config-file /tmp/tempo.yml --endpoint http://monitoring.internal --dry-run
```
