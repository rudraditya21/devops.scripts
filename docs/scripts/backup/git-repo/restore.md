...
## Purpose
Restore `git-repo` backup artifacts into a target path.

## Location
`backup/git-repo/restore.sh`
...
## Usage
```bash
backup/git-repo/restore.sh --input /backups/git-repo/app.tar.gz --target /restore/app
```
