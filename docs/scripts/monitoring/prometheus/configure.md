...
## Purpose
Generate service configuration for `prometheus` monitoring workflows.

## Location
`monitoring/prometheus/configure.sh`
...
## Usage
```bash
monitoring/prometheus/configure.sh --config-file /tmp/prometheus.yml --endpoint http://monitoring.internal --dry-run
```
