...
Apply an cloudformation infrastructure plan.
...
if $dry_run; then
  printf 'DRY-RUN: apply cloudformation plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
  exit 0
fi

printf 'Applied cloudformation plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
exit 0
