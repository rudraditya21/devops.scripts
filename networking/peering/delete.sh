...
Run delete for peering networking objects.
...
if $json; then
  printf '{"domain":"peering","action":"delete","name":"%s","target":"%s","cidr":"%s","dry_run":%s}\n' \
    "$(json_escape "$name")" \
    "$(json_escape "$target")" \
    "$(json_escape "$cidr")" \
    "$dry_run"
...
if $dry_run; then
  printf 'DRY-RUN: would delete peering object %s\n' "$name" >&2
fi
