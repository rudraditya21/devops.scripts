...
Apply postgres migrations and record state.

Options:
  --migrations-dir PATH    Directory with migration files (required)
  --target VERSION         Target version label (default: latest)
  --state-file PATH        State file path (default: .state/postgres-migrations.state)
...
migrations_dir=""
target="latest"
state_file=".state/postgres-migrations.state"
dry_run=false
