...
## Purpose
Sync dashboard files for `loki` monitoring workflows.

## Location
`monitoring/loki/dashboard-sync.sh`
...
## Usage
```bash
monitoring/loki/dashboard-sync.sh --source ./dashboards --destination /tmp/loki-dashboards --folder platform --dry-run
```
