...
## Purpose
Install component metadata for `prometheus` monitoring workflows.
...
| `--install-dir PATH` | No | `/opt/prometheus` | Install directory |
| `--config-dir PATH` | No | `/etc/prometheus` | Config directory |
| `--bin-dir PATH` | No | `/usr/local/bin` | Binary directory |
| `--dry-run` | No | `false` | Print actions only |

## Usage
```bash
monitoring/prometheus/install.sh --version v1.2.3 --dry-run
```
