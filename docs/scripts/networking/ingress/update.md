...
## Purpose
Run `update` for `ingress` networking objects using validated flags.

## Location
`networking/ingress/update.sh`
...
## Usage
```bash
networking/ingress/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
