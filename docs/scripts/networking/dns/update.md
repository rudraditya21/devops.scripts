...
## Purpose
Run `update` for `dns` networking objects using validated flags.

## Location
`networking/dns/update.sh`
...
## Usage
```bash
networking/dns/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
