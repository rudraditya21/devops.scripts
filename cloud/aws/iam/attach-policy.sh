...
usage() {
  cat << 'USAGE'
Usage: attach-policy.sh [OPTIONS]

Attach managed IAM policies to a user, role, or group.
...
Options:
  ...
  --if-attached        Skip if policy already attached (default behavior)
...
attach_policy() {
  ...
  case "$principal_type" in
    role)
      run_cmd aws "${aws_options[@]}" iam attach-role-policy --role-name "$principal_name" --policy-arn "$policy_arn"
      ;;
    user)
      run_cmd aws "${aws_options[@]}" iam attach-user-policy --user-name "$principal_name" --policy-arn "$policy_arn"
      ;;
    group)
      run_cmd aws "${aws_options[@]}" iam attach-group-policy --group-name "$principal_name" --policy-arn "$policy_arn"
      ;;
  esac
}
...
for arn in "${policy_arns[@]}"; do
  if principal_has_policy "$principal_type" "$principal_name" "$arn"; then
    if $skip_if_attached; then
      log "policy already attached, skipping: $arn"
      continue
    fi
    die "policy already attached: $arn"
  fi

  attach_policy "$principal_type" "$principal_name" "$arn"
  log "policy attached to ${principal_type} ${principal_name}: $arn"
done
