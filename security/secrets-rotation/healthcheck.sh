...
Run secrets-rotation security healthcheck.

Options:
  --target HOST           Hostname or IP (default: 127.0.0.1)
  --port N                TCP port (default: 8200)
...
if $json; then
  printf '{"domain":"secrets-rotation","target":"%s","port":%s,"status":"%s","message":"%s"}\n' "$target" "$port" "$status" "$message"
else
  printf 'domain: secrets-rotation\n'
  printf 'target: %s\n' "$target"
  printf 'port: %s\n' "$port"
  printf 'status: %s\n' "$status"
  printf 'message: %s\n' "$message"
