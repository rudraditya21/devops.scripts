...
## Purpose
Run `update` for `gateway` networking objects using validated flags.

## Location
`networking/gateway/update.sh`
...
## Usage
```bash
networking/gateway/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
