...
Stop one or more EC2 instances with optional hibernate/force and wait.
...
Options:
  ...
  --force              Force stop
  --hibernate          Attempt hibernation
  --wait               Wait until all instances are stopped (default)
...
wait_for_stopped() {
  ...
  while true; do
    ...
    if ((pending_count == 0)); then
      log "all instances reached stopped state"
      return 0
    fi

    elapsed=$(($(date +%s) - start_time))
    if ((elapsed >= timeout_seconds)); then
      die "timeout waiting for instances to reach stopped state"
    fi
...
while read -r instance_id instance_state; do
  ...
  case "$instance_state" in
    stopped | stopping)
      ...
      ;;
    running | pending)
      stop_targets+=("$instance_id")
      ;;
    *)
      die "instance $instance_id is in unsupported state for stop: $instance_state"
      ;;
  esac
done < <(aws "${aws_options[@]}" ec2 describe-instances \
  --instance-ids "${instance_ids[@]}" \
...
if ((${#stop_targets[@]} == 0)); then
  log "no instances require stop"
  exit 0
fi

stop_cmd=(aws "${aws_options[@]}" ec2 stop-instances --instance-ids "${stop_targets[@]}")
$force_stop && stop_cmd+=(--force)
$hibernate && stop_cmd+=(--hibernate)
run_cmd "${stop_cmd[@]}" > /dev/null
log "stop requested for: ${stop_targets[*]}"
