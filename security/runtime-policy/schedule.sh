...
Schedule runtime-policy security runs.

Options:
  --cron SPEC             Cron schedule expression (required)
  --timezone TZ           Timezone name (default: UTC)
  --state-file PATH       Schedule state path (default: .state/runtime-policy-schedule.state)
...
cron_spec=""
timezone="UTC"
state_file=".state/runtime-policy-schedule.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: schedule runtime-policy cron=%s timezone=%s\n' "$cron_spec" "$timezone"
  exit 0
fi
