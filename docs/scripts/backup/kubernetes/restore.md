...
## Purpose
Restore `kubernetes` backup artifacts into a target path.

## Location
`backup/kubernetes/restore.sh`
...
## Usage
```bash
backup/kubernetes/restore.sh --input /backups/kubernetes/app.tar.gz --target /restore/app
```
