...
Evaluate database utilization and suggest rightsizing action.
...
if $json; then
  printf '{"provider":"database","cpu_util":%s,"memory_util":%s,"cpu_target":%s,"memory_target":%s,"action":"%s","rationale":"%s"}\n' \
    "$cpu_util" "$mem_util" "$cpu_target" "$mem_target" "$action" "$rationale"
else
  printf 'provider: database\n'
  printf 'cpu_util: %s\n' "$cpu_util"
