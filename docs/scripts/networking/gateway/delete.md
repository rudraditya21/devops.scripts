...
## Purpose
Run `delete` for `gateway` networking objects using validated flags.

## Location
`networking/gateway/delete.sh`
...
## Usage
```bash
networking/gateway/delete.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
