...
Enforce iam-audit security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce iam-audit policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=iam-audit action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
