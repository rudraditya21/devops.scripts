...
Run health checks for incident SRE workflows.
...
if $json; then
  printf '{"kind":"incident","overall":"%s","checks":[{"name":"cmd:bash","status":"%s","detail":"%s"},{"name":"state-file","status":"%s","detail":"%s"}]}' \
    "$(json_escape "$overall")" \
