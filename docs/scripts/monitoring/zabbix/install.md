...
## Purpose
Install component metadata for `zabbix` monitoring workflows.

## Location
`monitoring/zabbix/install.sh`
...
| `--install-dir PATH` | No | `/opt/zabbix` | Install directory |
| `--config-dir PATH` | No | `/etc/zabbix` | Config directory |
| `--bin-dir PATH` | No | `/usr/local/bin` | Binary directory |
| `--dry-run` | No | `false` | Print actions only |

## Usage
```bash
monitoring/zabbix/install.sh --version v1.2.3 --dry-run
```
