...
## Purpose
Sync dashboard files for `elastic-stack` monitoring workflows.

## Location
`monitoring/elastic-stack/dashboard-sync.sh`
...
## Usage
```bash
monitoring/elastic-stack/dashboard-sync.sh --source ./dashboards --destination /tmp/elastic-stack-dashboards --folder platform --dry-run
```
