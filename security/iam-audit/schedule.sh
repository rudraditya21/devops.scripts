...
Schedule iam-audit security runs.

Options:
  --cron SPEC             Cron schedule expression (required)
  --timezone TZ           Timezone name (default: UTC)
  --state-file PATH       Schedule state path (default: .state/iam-audit-schedule.state)
...
cron_spec=""
timezone="UTC"
state_file=".state/iam-audit-schedule.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: schedule iam-audit cron=%s timezone=%s\n' "$cron_spec" "$timezone"
  exit 0
fi
