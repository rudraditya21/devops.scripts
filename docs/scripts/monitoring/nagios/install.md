...
## Purpose
Install component metadata for `nagios` monitoring workflows.

## Location
`monitoring/nagios/install.sh`
...
| `--install-dir PATH` | No | `/opt/nagios` | Install directory |
| `--config-dir PATH` | No | `/etc/nagios` | Config directory |
| `--bin-dir PATH` | No | `/usr/local/bin` | Binary directory |
| `--dry-run` | No | `false` | Print actions only |

## Usage
```bash
monitoring/nagios/install.sh --version v1.2.3 --dry-run
```
