...
Sync tag metadata from source to destination.
...
if $json; then
  printf '{"domain":"tag","action":"sync","source":"%s","destination":"%s","dry_run":%s,"status":"ok"}\n' \
...
if $dry_run; then
  printf 'DRY-RUN: would sync tag from %s to %s\n' "$source_path" "$destination_path" >&2
fi
