...
Enforce cert-management security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce cert-management policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=cert-management action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
