...
## Purpose
Generate service configuration for `loki` monitoring workflows.

## Location
`monitoring/loki/configure.sh`
...
## Usage
```bash
monitoring/loki/configure.sh --config-file /tmp/loki.yml --endpoint http://monitoring.internal --dry-run
```
