...
Run a disaster-recovery smoke test for object-storage backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/object-storage-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: object-storage\n'
printf 'workdir: %s\n' "$workdir"
