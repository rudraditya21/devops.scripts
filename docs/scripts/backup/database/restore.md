...
## Purpose
Restore `database` backup artifacts into a target path.

## Location
`backup/database/restore.sh`
...
## Usage
```bash
backup/database/restore.sh --input /backups/database/app.tar.gz --target /restore/app
```
