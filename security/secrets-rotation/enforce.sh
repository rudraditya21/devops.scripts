...
Enforce secrets-rotation security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce secrets-rotation policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=secrets-rotation action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
