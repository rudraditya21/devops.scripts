...
## Purpose
Sync dashboard files for `prometheus` monitoring workflows.

## Location
`monitoring/prometheus/dashboard-sync.sh`
...
## Usage
```bash
monitoring/prometheus/dashboard-sync.sh --source ./dashboards --destination /tmp/prometheus-dashboards --folder platform --dry-run
```
