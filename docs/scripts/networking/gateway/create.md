...
## Purpose
Run `create` for `gateway` networking objects using validated flags.

## Location
`networking/gateway/create.sh`
...
## Usage
```bash
networking/gateway/create.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
