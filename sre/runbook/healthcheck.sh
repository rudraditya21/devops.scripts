...
Run health checks for runbook SRE workflows.
...
if $json; then
  printf '{"kind":"runbook","overall":"%s","checks":[{"name":"cmd:bash","status":"%s","detail":"%s"},{"name":"state-file","status":"%s","detail":"%s"}]}' \
    "$(json_escape "$overall")" \
