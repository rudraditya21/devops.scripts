...
Install nagios monitoring components.

Options:
  --version VALUE      Version to record (default: latest)
  --channel NAME       Release channel (default: stable)
  --install-dir PATH   Install directory (default: /opt/nagios)
  --config-dir PATH    Config directory (default: /etc/nagios)
...
if $dry_run; then
  ...
  printf 'DRY-RUN: write install receipt for nagios version=%s channel=%s\n' "$version" "$channel"
  exit 0
fi
...
printf 'Installed nagios metadata at %s\n' "$install_dir/INSTALL_RECEIPT"
exit 0
