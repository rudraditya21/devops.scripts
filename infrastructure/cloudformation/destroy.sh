...
Destroy cloudformation managed infrastructure.
...
if $dry_run; then
  printf 'DRY-RUN: destroy cloudformation workspace=%s target=%s\n' "$workspace" "$target"
  exit 0
fi

printf 'Destroyed cloudformation workspace=%s target=%s\n' "$workspace" "$target"
exit 0
