...
Install zabbix monitoring components.

Options:
  --version VALUE      Version to record (default: latest)
  --channel NAME       Release channel (default: stable)
  --install-dir PATH   Install directory (default: /opt/zabbix)
  --config-dir PATH    Config directory (default: /etc/zabbix)
...
if $dry_run; then
  ...
  printf 'DRY-RUN: write install receipt for zabbix version=%s channel=%s\n' "$version" "$channel"
  exit 0
fi
...
printf 'Installed zabbix metadata at %s\n' "$install_dir/INSTALL_RECEIPT"
exit 0
