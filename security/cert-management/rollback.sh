...
Rollback cert-management security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/cert-management.state)
...
change_id=""
state_file=".state/cert-management.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback cert-management change=%s\n' "$change_id"
  exit 0
fi
