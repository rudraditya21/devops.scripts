...
Install grafana monitoring components.

Options:
  --version VALUE      Version to record (default: latest)
  --channel NAME       Release channel (default: stable)
  --install-dir PATH   Install directory (default: /opt/grafana)
  --config-dir PATH    Config directory (default: /etc/grafana)
...
if $dry_run; then
  ...
  printf 'DRY-RUN: write install receipt for grafana version=%s channel=%s\n' "$version" "$channel"
  exit 0
fi
...
printf 'Installed grafana metadata at %s\n' "$install_dir/INSTALL_RECEIPT"
exit 0
