...
## Purpose
Verify presence and integrity of `vm` backup artifacts.
...
## Usage
```bash
backup/vm/verify.sh --backup /backups/vm/app.tar.gz
backup/vm/verify.sh --backup /backups/vm/app.tar.gz --json
```
