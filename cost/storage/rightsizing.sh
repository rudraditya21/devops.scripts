...
Evaluate storage utilization and suggest rightsizing action.
...
if $json; then
  printf '{"provider":"storage","cpu_util":%s,"memory_util":%s,"cpu_target":%s,"memory_target":%s,"action":"%s","rationale":"%s"}\n' \
