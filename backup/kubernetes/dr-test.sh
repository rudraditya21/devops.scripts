...
Run a disaster-recovery smoke test for kubernetes backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/kubernetes-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: kubernetes\n'
printf 'workdir: %s\n' "$workdir"
