...
Evaluate azure utilization and suggest rightsizing action.
...
if $json; then
  printf '{"provider":"azure","cpu_util":%s,"memory_util":%s,"cpu_target":%s,"memory_target":%s,"action":"%s","rationale":"%s"}\n' \
