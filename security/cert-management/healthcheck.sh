...
Run cert-management security healthcheck.

Options:
  --target HOST           Hostname or IP (default: 127.0.0.1)
  --port N                TCP port (default: 8443)
...
if $json; then
  printf '{"domain":"cert-management","target":"%s","port":%s,"status":"%s","message":"%s"}\n' "$target" "$port" "$status" "$message"
else
  printf 'domain: cert-management\n'
  printf 'target: %s\n' "$target"
  printf 'port: %s\n' "$port"
  printf 'status: %s\n' "$status"
  printf 'message: %s\n' "$message"
