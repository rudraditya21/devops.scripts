...
Apply an ansible infrastructure plan.
...
if $dry_run; then
  printf 'DRY-RUN: apply ansible plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
  exit 0
fi

printf 'Applied ansible plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
exit 0
