...
## Purpose
Run `update` for `vpn` networking objects using validated flags.

## Location
`networking/vpn/update.sh`
...
## Usage
```bash
networking/vpn/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
