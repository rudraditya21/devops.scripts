...
Export gcp cost optimization recommendations.
...
if [[ "$format" == "csv" ]]; then
  ...
  i=1
  while ((i <= top)); do
    content+="rec-${i},gcp,resource-${i},optimize-size-${i},$((i * 25))\n"
    i=$((i + 1))
  done
else
  content='['
  i=1
  while ((i <= top)); do
    item=$(printf '{"id":"rec-%s","provider":"gcp","resource":"resource-%s","recommendation":"optimize-size-%s","estimated_monthly_savings":%s}' "$i" "$i" "$i" "$((i * 25))")
    if ((i > 1)); then
      content+=','
    fi
    content+="$item"
