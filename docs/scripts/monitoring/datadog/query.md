...
## Purpose
Run normalized metric/log/trace query for `datadog` monitoring workflows.

## Location
`monitoring/datadog/query.sh`
...
## Usage
```bash
monitoring/datadog/query.sh --expr 'up{job="api"}' --format json
```
