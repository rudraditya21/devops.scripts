...
## Purpose
Sync alert rule files for `elastic-stack` monitoring workflows.

## Location
`monitoring/elastic-stack/alert-sync.sh`
...
## Usage
```bash
monitoring/elastic-stack/alert-sync.sh --source ./alerts --destination /tmp/elastic-stack-alerts --dry-run
```
