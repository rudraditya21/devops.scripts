...
usage() {
  cat << 'USAGE'
Usage: bootstrap-macos-runner.sh [OPTIONS]

Bootstrap a macOS runner with local setup scripts.
...
log() {
  printf '%s [bootstrap-macos-runner] %s\n' "$(date +"%Y-%m-%dT%H:%M:%S%z")" "$*" >&2
}
...
if ! command -v brew > /dev/null 2>&1; then
  if ! $install_brew; then
    die "Homebrew is required on macOS (or pass --install-brew)"
  fi
...
if ! $skip_docker; then
  run_step "install docker" bash "$runner_setup_dir/install-docker.sh" "${common_flags[@]}" --no-start-service
fi
...
log "macOS runner bootstrap completed"
