...
Detect storage spend anomalies from baseline and current values.
...
if $json; then
  printf '{"provider":"storage","baseline":%s,"current":%s,"deviation_percent":%s,"threshold_percent":%s,"status":"%s"}\n' \
