...
Lint cloudformation infrastructure files.
...
if [[ "$format" == "json" ]]; then
  printf '{"stack":"cloudformation","action":"lint","config_dir":"%s","warnings":0,"max_warnings":%s,"status":"ok"}\n' "$config_dir" "$max_warnings"
else
  printf 'stack: cloudformation\n'
  printf 'action: lint\n'
  printf 'config_dir: %s\n' "$config_dir"
  printf 'warnings: 0\n'
