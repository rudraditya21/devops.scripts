...
## Purpose
Sync dashboard files for `nagios` monitoring workflows.

## Location
`monitoring/nagios/dashboard-sync.sh`
...
## Usage
```bash
monitoring/nagios/dashboard-sync.sh --source ./dashboards --destination /tmp/nagios-dashboards --folder platform --dry-run
```
