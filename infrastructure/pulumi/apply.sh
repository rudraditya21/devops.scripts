...
Apply an pulumi infrastructure plan.
...
if $dry_run; then
  printf 'DRY-RUN: apply pulumi plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
  exit 0
fi

printf 'Applied pulumi plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
exit 0
