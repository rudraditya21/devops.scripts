...
Rollback signing security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/signing.state)
...
change_id=""
state_file=".state/signing.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback signing change=%s\n' "$change_id"
  exit 0
fi
