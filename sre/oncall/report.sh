...
Generate oncall SRE report data.
...
if [[ "$format" == "json" ]]; then
  payload=$(printf '{"kind":"oncall","period":"%s","owner":"%s","summary":{"ok":3,"warn":1,"critical":0}}' \
    "$(json_escape "$period")" \
    "$(json_escape "$owner")")
else
  payload=$(cat <<TABLE
...
if [[ -n "$output_path" ]]; then
  if $dry_run; then
    printf 'DRY-RUN: would write oncall report to %s\n' "$output_path" >&2
  else
