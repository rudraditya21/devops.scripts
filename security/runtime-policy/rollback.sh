...
Rollback runtime-policy security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/runtime-policy.state)
...
change_id=""
state_file=".state/runtime-policy.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback runtime-policy change=%s\n' "$change_id"
  exit 0
fi
