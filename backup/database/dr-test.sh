...
Run a disaster-recovery smoke test for database backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/database-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: database\n'
printf 'workdir: %s\n' "$workdir"
