...
Rollback iam-audit security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/iam-audit.state)
...
change_id=""
state_file=".state/iam-audit.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback iam-audit change=%s\n' "$change_id"
  exit 0
fi
