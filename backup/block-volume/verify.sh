...
Verify integrity and presence of a block-volume backup artifact.
...
if $json; then
  printf '{"category":"block-volume","backup":"%s","status":"%s","exists":%s,"non_empty":%s,"checksum":"%s","detail":"%s"}\n' \
