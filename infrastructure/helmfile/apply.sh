...
Apply an helmfile infrastructure plan.
...
if $dry_run; then
  printf 'DRY-RUN: apply helmfile plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
  exit 0
fi

printf 'Applied helmfile plan=%s workspace=%s auto_approve=%s\n' "$plan_file" "$workspace" "$auto_approve"
exit 0
