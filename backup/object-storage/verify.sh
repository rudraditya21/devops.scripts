...
Verify integrity and presence of a object-storage backup artifact.
...
if $json; then
  printf '{"category":"object-storage","backup":"%s","status":"%s","exists":%s,"non_empty":%s,"checksum":"%s","detail":"%s"}\n' \
