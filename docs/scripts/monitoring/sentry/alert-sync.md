...
## Purpose
Sync alert rule files for `sentry` monitoring workflows.

## Location
`monitoring/sentry/alert-sync.sh`
...
## Usage
```bash
monitoring/sentry/alert-sync.sh --source ./alerts --destination /tmp/sentry-alerts --dry-run
```
