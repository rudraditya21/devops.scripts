...
## Purpose
Sync alert rule files for `grafana` monitoring workflows.

## Location
`monitoring/grafana/alert-sync.sh`
...
## Usage
```bash
monitoring/grafana/alert-sync.sh --source ./alerts --destination /tmp/grafana-alerts --dry-run
```
