...
## Purpose
Sync alert rule files for `datadog` monitoring workflows.

## Location
`monitoring/datadog/alert-sync.sh`
...
## Usage
```bash
monitoring/datadog/alert-sync.sh --source ./alerts --destination /tmp/datadog-alerts --dry-run
```
