...
## Purpose
Run `update` for `firewall` networking objects using validated flags.

## Location
`networking/firewall/update.sh`
...
## Usage
```bash
networking/firewall/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
