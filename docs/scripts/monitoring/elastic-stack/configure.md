...
## Purpose
Generate service configuration for `elastic-stack` monitoring workflows.

## Location
`monitoring/elastic-stack/configure.sh`
...
## Usage
```bash
monitoring/elastic-stack/configure.sh --config-file /tmp/elastic-stack.yml --endpoint http://monitoring.internal --dry-run
```
