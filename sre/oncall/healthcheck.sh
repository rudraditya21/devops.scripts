...
Run health checks for oncall SRE workflows.
...
if $json; then
  printf '{"kind":"oncall","overall":"%s","checks":[{"name":"cmd:bash","status":"%s","detail":"%s"},{"name":"state-file","status":"%s","detail":"%s"}]}' \
    "$(json_escape "$overall")" \
