...
## Purpose
Install component metadata for `loki` monitoring workflows.
...
| `--install-dir PATH` | No | `/opt/loki` | Install directory |
| `--config-dir PATH` | No | `/etc/loki` | Config directory |
| `--bin-dir PATH` | No | `/usr/local/bin` | Binary directory |
| `--dry-run` | No | `false` | Print actions only |

## Usage
```bash
monitoring/loki/install.sh --version v1.2.3 --dry-run
```
