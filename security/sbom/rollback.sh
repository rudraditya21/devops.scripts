...
Rollback sbom security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/sbom.state)
...
if $dry_run; then
  printf 'DRY-RUN: rollback sbom change=%s\n' "$change_id"
  exit 0
fi
