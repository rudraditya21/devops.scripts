...
## Purpose
Sync dashboard files for `zabbix` monitoring workflows.

## Location
`monitoring/zabbix/dashboard-sync.sh`
...
## Usage
```bash
monitoring/zabbix/dashboard-sync.sh --source ./dashboards --destination /tmp/zabbix-dashboards --folder platform --dry-run
```
