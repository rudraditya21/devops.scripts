...
Create commit metadata/object scaffolding.
...
if $json; then
  printf '{"domain":"commit","action":"create","name":"%s","metadata_count":%s,"dry_run":%s}\n' \
...
if $dry_run; then
  printf 'DRY-RUN: would create commit object %s\n' "$name" >&2
fi
