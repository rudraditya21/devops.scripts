...
Run update for ingress networking objects.
...
if $json; then
  printf '{"domain":"ingress","action":"update","name":"%s","target":"%s","cidr":"%s","dry_run":%s}\n' \
    "$(json_escape "$name")" \
    "$(json_escape "$target")" \
    "$(json_escape "$cidr")" \
    "$dry_run"
...
if $dry_run; then
  printf 'DRY-RUN: would update ingress object %s\n' "$name" >&2
fi
