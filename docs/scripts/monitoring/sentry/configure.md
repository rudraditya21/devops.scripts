...
## Purpose
Generate service configuration for `sentry` monitoring workflows.

## Location
`monitoring/sentry/configure.sh`
...
## Usage
```bash
monitoring/sentry/configure.sh --config-file /tmp/sentry.yml --endpoint http://monitoring.internal --dry-run
```
