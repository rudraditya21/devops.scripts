...
## Purpose
Run `create` for `loadbalancer` networking objects using validated flags.

## Location
`networking/loadbalancer/create.sh`
...
## Usage
```bash
networking/loadbalancer/create.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
