...
## Purpose
Verify presence and integrity of `filesystem` backup artifacts.

## Location
`backup/filesystem/verify.sh`
...
## Usage
```bash
backup/filesystem/verify.sh --backup /backups/filesystem/app.tar.gz
backup/filesystem/verify.sh --backup /backups/filesystem/app.tar.gz --json
```
