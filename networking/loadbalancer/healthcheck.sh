...
Run a basic healthcheck for loadbalancer networking workflows.
...
if $json; then
  printf '{"domain":"loadbalancer","overall":"%s","checks":[{"name":"cmd:bash","status":"%s","detail":"%s"},{"name":"endpoint","status":"%s","detail":"%s"}]}\n' \
    "$(json_escape "$overall")" \
