...
Install elastic-stack monitoring components.

Options:
  --version VALUE      Version to record (default: latest)
  --channel NAME       Release channel (default: stable)
  --install-dir PATH   Install directory (default: /opt/elastic-stack)
  --config-dir PATH    Config directory (default: /etc/elastic-stack)
...
if $dry_run; then
  ...
  printf 'DRY-RUN: write install receipt for elastic-stack version=%s channel=%s\n' "$version" "$channel"
  exit 0
fi
...
printf 'Installed elastic-stack metadata at %s\n' "$install_dir/INSTALL_RECEIPT"
exit 0
