...
Install tempo monitoring components.

Options:
  --version VALUE      Version to record (default: latest)
  --channel NAME       Release channel (default: stable)
  --install-dir PATH   Install directory (default: /opt/tempo)
  --config-dir PATH    Config directory (default: /etc/tempo)
...
if $dry_run; then
  ...
  printf 'DRY-RUN: write install receipt for tempo version=%s channel=%s\n' "$version" "$channel"
  exit 0
fi
...
printf 'Installed tempo metadata at %s\n' "$install_dir/INSTALL_RECEIPT"
exit 0
