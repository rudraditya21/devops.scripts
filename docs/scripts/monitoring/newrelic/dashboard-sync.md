...
## Purpose
Sync dashboard files for `newrelic` monitoring workflows.

## Location
`monitoring/newrelic/dashboard-sync.sh`
...
## Usage
```bash
monitoring/newrelic/dashboard-sync.sh --source ./dashboards --destination /tmp/newrelic-dashboards --folder platform --dry-run
```
