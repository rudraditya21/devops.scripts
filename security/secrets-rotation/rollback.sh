...
Rollback secrets-rotation security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/secrets-rotation.state)
...
change_id=""
state_file=".state/secrets-rotation.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback secrets-rotation change=%s\n' "$change_id"
  exit 0
fi
