...
## Purpose
Run `update` for `egress` networking objects using validated flags.

## Location
`networking/egress/update.sh`
...
## Usage
```bash
networking/egress/update.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
