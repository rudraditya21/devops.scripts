...
Enforce signing security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce signing policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=signing action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
