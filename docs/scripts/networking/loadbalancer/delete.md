...
## Purpose
Run `delete` for `loadbalancer` networking objects using validated flags.

## Location
`networking/loadbalancer/delete.sh`
...
## Usage
```bash
networking/loadbalancer/delete.sh --name app-edge --target prod --cidr 10.10.0.0/24 --dry-run
```
