...
Start one or more GCP Compute Engine instances.
...
Options:
  ...
  --wait                  Wait for RUNNING state (default)
...
wait_for_running() {
  ...
  while true; do
    ...
    if ((elapsed >= timeout_seconds)); then
      die "timeout waiting for instances to reach RUNNING"
    fi
...
for name in "${instance_names[@]}"; do
  run_cmd gcloud compute instances start "$name" --zone "$zone" "${gcloud_opts[@]}"
done

log "start requested for: ${instance_names[*]}"
...
if $wait_enabled; then
  if $dry_run; then
    ...
    wait_for_running
    log "all instances reached RUNNING"
  fi
fi
