...
Schedule secrets-rotation security runs.

Options:
  --cron SPEC             Cron schedule expression (required)
  --timezone TZ           Timezone name (default: UTC)
  --state-file PATH       Schedule state path (default: .state/secrets-rotation-schedule.state)
...
cron_spec=""
timezone="UTC"
state_file=".state/secrets-rotation-schedule.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: schedule secrets-rotation cron=%s timezone=%s\n' "$cron_spec" "$timezone"
  exit 0
fi

mkdir -p "$(dirname "$state_file")"
cat > "$state_file" <<STATE
domain=secrets-rotation
cron=$cron_spec
timezone=$timezone
