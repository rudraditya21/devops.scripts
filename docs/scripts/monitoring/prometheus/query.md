...
## Purpose
Run normalized metric/log/trace query for `prometheus` monitoring workflows.

## Location
`monitoring/prometheus/query.sh`
...
## Usage
```bash
monitoring/prometheus/query.sh --expr 'up{job="api"}' --format json
```
