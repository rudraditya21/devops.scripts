...
## Purpose
Run normalized metric/log/trace query for `newrelic` monitoring workflows.

## Location
`monitoring/newrelic/query.sh`
...
## Usage
```bash
monitoring/newrelic/query.sh --expr 'up{job="api"}' --format json
```
