...
## Purpose
Verify presence and integrity of `kubernetes` backup artifacts.

## Location
`backup/kubernetes/verify.sh`
...
## Usage
```bash
backup/kubernetes/verify.sh --backup /backups/kubernetes/app.tar.gz
backup/kubernetes/verify.sh --backup /backups/kubernetes/app.tar.gz --json
```
