...
## Purpose
Generate service configuration for `datadog` monitoring workflows.

## Location
`monitoring/datadog/configure.sh`
...
## Usage
```bash
monitoring/datadog/configure.sh --config-file /tmp/datadog.yml --endpoint http://monitoring.internal --dry-run
```
