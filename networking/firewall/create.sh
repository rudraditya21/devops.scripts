...
Run create for firewall networking objects.
...
if $json; then
  printf '{"domain":"firewall","action":"create","name":"%s","target":"%s","cidr":"%s","dry_run":%s}\n' \
    "$(json_escape "$name")" \
    "$(json_escape "$target")" \
    "$(json_escape "$cidr")" \
    "$dry_run"
...
if $dry_run; then
  printf 'DRY-RUN: would create firewall object %s\n' "$name" >&2
fi
