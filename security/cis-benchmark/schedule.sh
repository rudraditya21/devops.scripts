...
Schedule cis-benchmark security runs.

Options:
  --cron SPEC             Cron schedule expression (required)
  --timezone TZ           Timezone name (default: UTC)
  --state-file PATH       Schedule state path (default: .state/cis-benchmark-schedule.state)
...
cron_spec=""
timezone="UTC"
state_file=".state/cis-benchmark-schedule.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: schedule cis-benchmark cron=%s timezone=%s\n' "$cron_spec" "$timezone"
  exit 0
fi

mkdir -p "$(dirname "$state_file")"
cat > "$state_file" <<STATE
domain=cis-benchmark
cron=$cron_spec
timezone=$timezone
