...
## Purpose
Run normalized metric/log/trace query for `grafana` monitoring workflows.

## Location
`monitoring/grafana/query.sh`
...
## Usage
```bash
monitoring/grafana/query.sh --expr 'up{job="api"}' --format json
```
