...
Enforce compliance security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce compliance policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=compliance action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
