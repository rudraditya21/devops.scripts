...
Run a disaster-recovery smoke test for filesystem backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/filesystem-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: filesystem\n'
printf 'workdir: %s\n' "$workdir"
