...
Sync error-budget SRE metadata between locations.
...
if $json; then
  printf '{"kind":"error-budget","source":"%s","destination":"%s","dry_run":%s,"status":"ok"}\n' \
...
if $dry_run; then
  printf 'DRY-RUN: would sync error-budget from %s to %s\n' "$source_path" "$dest_path" >&2
fi
