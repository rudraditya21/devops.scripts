...
Enforce runtime-policy security policy.
...
if $dry_run; then
  printf 'DRY-RUN: enforce runtime-policy policy=%s mode=%s target=%s\n' "$policy_file" "$mode" "$target"
  exit 0
fi

printf 'domain=runtime-policy action=enforce policy=%s mode=%s target=%s status=applied\n' "$policy_file" "$mode" "$target"
exit 0
