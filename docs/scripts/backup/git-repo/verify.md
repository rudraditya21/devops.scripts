...
## Purpose
Verify presence and integrity of `git-repo` backup artifacts.

## Location
`backup/git-repo/verify.sh`
...
## Usage
```bash
backup/git-repo/verify.sh --backup /backups/git-repo/app.tar.gz
backup/git-repo/verify.sh --backup /backups/git-repo/app.tar.gz --json
```
