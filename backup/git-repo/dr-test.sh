...
Run a disaster-recovery smoke test for git-repo backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/git-repo-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: git-repo\n'
printf 'workdir: %s\n' "$workdir"
