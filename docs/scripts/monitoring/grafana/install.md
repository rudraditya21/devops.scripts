...
## Purpose
Install component metadata for `grafana` monitoring workflows.
...
| `--install-dir PATH` | No | `/opt/grafana` | Install directory |
| `--config-dir PATH` | No | `/etc/grafana` | Config directory |
| `--bin-dir PATH` | No | `/usr/local/bin` | Binary directory |
| `--dry-run` | No | `false` | Print actions only |

## Usage
```bash
monitoring/grafana/install.sh --version v1.2.3 --dry-run
```
