...
Sync commit metadata from source to destination.
...
if $json; then
  printf '{"domain":"commit","action":"sync","source":"%s","destination":"%s","dry_run":%s,"status":"ok"}\n' \
...
if $dry_run; then
  printf 'DRY-RUN: would sync commit from %s to %s\n' "$source_path" "$destination_path" >&2
fi
