...
## Purpose
Sync alert rule files for `prometheus` monitoring workflows.

## Location
`monitoring/prometheus/alert-sync.sh`
...
## Usage
```bash
monitoring/prometheus/alert-sync.sh --source ./alerts --destination /tmp/prometheus-alerts --dry-run
```
