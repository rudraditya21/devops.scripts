...
Run health checks for slo SRE workflows.
...
if $json; then
  printf '{"kind":"slo","overall":"%s","checks":[{"name":"cmd:bash","status":"%s","detail":"%s"},{"name":"state-file","status":"%s","detail":"%s"}]}' \
    "$(json_escape "$overall")" \
