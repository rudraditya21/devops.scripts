...
Run a disaster-recovery smoke test for block-volume backups.
...
if [[ -z "$workdir" ]]; then
  workdir="$(mktemp -d "${TMPDIR:-/tmp}/block-volume-dr-test.XXXXXX")"
else
  mkdir -p "$workdir"
fi
...
printf 'DR test completed for category: block-volume\n'
printf 'workdir: %s\n' "$workdir"
