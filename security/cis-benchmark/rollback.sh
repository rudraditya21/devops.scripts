...
Rollback cis-benchmark security changes.

Options:
  --change-id ID          Change identifier (required)
  --state-file PATH       State file path (default: .state/cis-benchmark.state)
...
change_id=""
state_file=".state/cis-benchmark.state"
dry_run=false
...
if $dry_run; then
  printf 'DRY-RUN: rollback cis-benchmark change=%s\n' "$change_id"
  exit 0
fi
