...
## Purpose
Generate service configuration for `nagios` monitoring workflows.

## Location
`monitoring/nagios/configure.sh`
...
## Usage
```bash
monitoring/nagios/configure.sh --config-file /tmp/nagios.yml --endpoint http://monitoring.internal --dry-run
```
